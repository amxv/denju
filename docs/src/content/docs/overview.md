...
The Rust implementation provides deterministic Agent Skills validation and content identity, anonymous `denju setup`, unified metadata-only local/private/team/public search, profiles/follows/stars/rankings/reports, account-wide direct subscriptions, reproducible versioned packs with durable follow-latest application, verified cold installs from S3-compatible storage, optional password/recovery identity claim and login, authenticated private skill import, automatic and explicit forks with immutable provenance, private sharing and live private subscriptions, private moving proposals that accept into upstream private history without publishing, editable owned workspaces with durable private revisions, deterministic multi-device merges and explicit conflict resolution, shared team namespaces with role-based publishing and maintainer-private draft refs, stable-ID personal-to-team transfer, stable-ID rename/unpublish/delete/deprecation and explicit history pruning, revocable devices and scoped automation credentials, durable SQLite-backed local state, Codex and Claude projections, per-user service management, and the PostgreSQL-backed registry foundation used by the CLI. See [Social discovery and search](/docs/social-discovery) for the discovery boundary and its no-body-indexing guarantee.
...
Team-owned skills use the same revision machinery without introducing a shared live draft. Every authorized publisher has a private team workspace ref. Team members read immutable team releases, while a publisher sees and edits only their own current draft. A publish reconciles that publisher's known head against the latest team release, automatically merges compatible changes, and preserves both heads when explicit conflict resolution is required.
