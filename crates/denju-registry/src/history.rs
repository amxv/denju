...
use crate::{
    ...
    ingest::decode_32,
    internal_api_error,
    revision_graph::revision_parents,
};
...
impl Registry {
    pub async fn skill_history(
        ...
        let releases = self.history_release_rows(resource_id).await?;
        ...
        for bytes in rows {
            ...
            let readable = if private_read {
                true
            } else if let Some(access) = access.as_ref() {
                user_can_read_revision(&self.pool, access, resource_id, &revision_bytes).await?
            } else {
            ...
            revisions.push(SkillRevisionSummary {
                parent_revision_ids: revision_parents(&self.pool, &revision).await?,
        ...
        {
            workspace_head_for_user(&self.pool, resource_id, access.user_id).await?
        } else if private_read {
            match owner_workspace_head(&self.pool, resource_id).await? {
                Some(head) => head,
